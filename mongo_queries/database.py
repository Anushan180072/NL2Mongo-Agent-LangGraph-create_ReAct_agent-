...
class MongoDBDatabase:
    ...
    def from_connection_string(
        ...
        **kwargs: Any,
        ...
        print("**monogoconnection establishe**")
        client = MongoClient(
            connection_string,
    ...
    def get_collection_info(self, collection_names: Optional[List[str]] = None) -> str:
        ...
        for coll in all_coll_names:
            # add schema
            schema = self._schema             #self._get_collection_schema(coll) + self._schema
    ...
    def _get_sample_docs(self, collection: str) -> str:
        ...
        #docs = list(col.find({}, sort=[("_id", -1)], limit=1))
        #for doc in docs:
            #self._elide_doc(doc)
        return (
            f"1 sample document from {collection} collection:\n"
            """[
  {
    "company": {
      "$oid": "656ec2ff633f208bd8e75219"
    },
    "templates_fields_data": {
      "683c317cbb9177f8bc589717#product_id": "004",
      "683c317cbb9177f8bc589717#product_name": "Apples",
      "683c317cbb9177f8bc589717#category_id": "683c3185bb9177f8bc589839",
      "683c317cbb9177f8bc589717#supplier_id": "683c318fbb9177f8bc589882",
      "683c317cbb9177f8bc589717#unit_price": 1,
      "683c317cbb9177f8bc589717#description": "Fresh apples",
      "683c317cbb9177f8bc589717#image": "apples.jpg",
      "683c317cbb9177f8bc589717#product_id_info": {
        "number": "001"
      },
      "683c317cbb9177f8bc589717#category_id/name": "004",
      "683c317cbb9177f8bc589717#supplier_id/name": "004",
      "683c317cbb9177f8bc589717#unit_price_currency": "USD"
    }
  }
]"""
        )
    ...
    def _parse_command(self, command: str) -> Any:
        ...
        # Handle ISODate
        command = re.sub(r'ISODate\("([^"]+)"\)', r'{"$date": "\1"}', command)
    
        # Handle ObjectId
        command = re.sub(r'ObjectId\("([^"]+)"\)', r'{"$oid": "\1"}', command)
        ...
        try:
            return json_util.loads(agg_command)
        except Exception as e:
            raise ValueError(f"Cannot execute command {command} because {str(e)}") from e
